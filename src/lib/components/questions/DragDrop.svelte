  ...
  */
  ...
  function corriger(){
    ...
    while (i < boxQuestions.length) {
      ...
      liste.push(tab)
      /*tab[options.drag[0]] == boxReponses.dataset.reponse
      console.log(tab[options.drag[0]])
      console.log(boxReponses.dataset.reponse)*/
      i++
    }
    i = 0
    while (i < boxQuestions.length) {
    ...
    /*boxQuestions[0].map((boxQuestion) => {
      console.log(boxQuestion.dataset.question)
    })*/
  ...
  <div class="grid grid-cols-2 gap-1">
    {#if dragAlea}
      {#each dragAlea as drag}
        <div class="p-1 draggable d2 cursor-move select-none" draggable="true" on:dragstart={dDrag} on:dragend={fDrag} data-reponse={drag.tempDrag[0]}>
